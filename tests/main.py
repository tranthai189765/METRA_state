...
def make_env(args, max_path_length):
    if args.env == 'maze':
        ...
        elif args.env == 'dmc_cheetah_state':
            # cheetah_run_forward: obs_type=states, action_repeat=1 — matches TIME pretrain default config
            env = dmc.make('cheetah_run_forward', obs_type='states', frame_stack=1, action_repeat=1, seed=args.seed)
        else:
